...
- When a probe/test/validator fails during repair work, do not jump straight to a guessed fix. First classify the failure against: (1) relevant lessons learned and recent milestone evidence, (2) the actual shape and type/API boundaries of the working code path, and (3) the specific artifact/log output from the failed probe. Prefer the smallest source-level correction that preserves the gate; do not weaken validators, post-process built output, or repeat broad probes until the failure classification explains why the next probe should differ.
...
- Observation harness hard rule: every long or failure-prone proof must have an explicit observation harness contract before execution. If the milestone permits a detached observer, use the canonical deterministic harness (`scripts/long_running_cron_observer_harness.py` or successor) plus an independent completion check. If the milestone forbids detachment, embed the observation harness semantics into the foreground parent: periodic phase/liveness/PID/port/status receipts, owned observer child only if needed, same process group, captured PID, explicit termination and `wait`, and never let observer-launch PASS substitute for proof PASS.
- Critical apply hard rule: no critical production apply/deploy/install/recovery/runtime mutation may be governed by a foreground chat/session process or raw foreground approval exec. Foreground turns may prepare evidence and request permission only. The mutation must run as a durable observation-harness transaction with a maintenance lock, restore point fresh within 1 hour (or create/verify one first), pre/post sentinels, independent watcher/check, child PID/process-group receipts, stdout/stderr artifacts, postcheck, and automatic recovery classification. If an apply fails and the npm/package base is missing/empty/incomplete, the observer must inspect `.openclaw-*` process references and restore/reinstall only from the verified restore point within its authority. Install/apply, restart, and functional smoke are separate transactions. If no critical apply runner exists, stop at architecture/prep and do not improvise a foreground mutation. Architecture: `memory/critical-apply-observation-harness-architecture-2026-07-30.md`.
- No critical production apply, deploy, install, repair, restore, rollback, restart, activation, migration, or runtime mutation may be governed by a foreground chat/session process. If the HRL-3 Critical Apply service or required promoted plugin is absent or unproven, stop at architecture, preparation, fixtures, shadow validation, or read-only inspection.
