...
document.getElementById('send').onclick = async () => {
  ...
  const liveCue = j.voicePlan?.liveProsodyCueLayer?.moodScore || null;
  const liveCueLine = liveCue
    ? `<br><span class="muted">Live prosody: ${escapeHtml(liveCue.expressionToken || '🛰️')} ${escapeHtml(liveCue.moodLabel || liveCue.moodId || 'n/a')}; intensity ${escapeHtml(liveCue.averageIntensity ?? 'n/a')}; cues ${escapeHtml(liveCue.cueSequence || 'n/a')}; ${escapeHtml(j.voicePlan.liveProsodyCueLayer?.classification || 'cue-layer')}</span>`
    : '';
  const voicePlan = j.voicePlan?.delivery
    ? `<br><span class="muted">Voice score: mood ${escapeHtml(j.voicePlan.delivery.moodLabel || j.voicePlan.delivery.moodId || 'n/a')}, base temp ${escapeHtml(j.voicePlan.delivery.baseXttsParams?.temperature ?? 'n/a')}, effective temp ${escapeHtml(j.voicePlan.delivery.xttsParams?.temperature ?? 'n/a')}, top_p ${escapeHtml(j.voicePlan.delivery.xttsParams?.topP ?? 'n/a')}, XTTS speed ${escapeHtml(j.voicePlan.delivery.xttsParams?.speed ?? 'n/a')}, chunk ${escapeHtml(j.voicePlan.delivery.maxCharsPerChunk || 'n/a')}</span>${liveCueLine}${selectedScoreChunk ? `<br><span class="muted">Selected chunk: ${escapeHtml(selectedScoreChunk.chunkId || 'c001')} role ${escapeHtml(selectedScoreChunk.phraseRole || 'n/a')}; Δ ${escapeHtml(JSON.stringify(selectedScoreChunk.deltas || {}))}; effective ${escapeHtml(JSON.stringify(selectedScoreChunk.effectiveXtts || selectedScoreChunk.xttsParams || {}))}</span>` : ''}${hasStreamingFrames ? `<br><span class="muted">Streaming frames: ${escapeHtml(realtimeFrames.length)} / target ${escapeHtml(j.voicePlan.audioPerformance?.realtime?.target || 'streaming_full_duplex_mesh')}</span>` : ''}`
    : '';
